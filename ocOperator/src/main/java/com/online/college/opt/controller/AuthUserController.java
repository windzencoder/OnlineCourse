...
public class AuthUserController {
	...
	public ModelAndView queryPage(AuthUser queryEntity, TailPage<AuthUser> page) {
		...
		if (StringUtils.isNotEmpty(queryEntity.getUsername())) {//可能会根据username进行搜索
			queryEntity.setUsername(queryEntity.getUsername().trim());//去掉空格
		} else {
			queryEntity.setUsername(null);
		}

		if (Integer.valueOf(-1).equals(queryEntity.getStatus())) {//选择"全部"
			queryEntity.setStatus(null);
		}

		page = entityService.queryPage(queryEntity, page);
		mv.addObject("page", page);
		mv.addObject("queryEntity", queryEntity);//保存查询条件
