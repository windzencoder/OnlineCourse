...
//添加注解，代表spring会加载这个注解
@Component
public class ClassfiyBusinessImpl implements IClassifyBusiness{
	...
	public Map<String, ClassifyDto> getAllClassify() {
		...
		while (it.hasNext()) {
			ConstsClassify item = (ConstsClassify) it.next();
			if ("0".equals(item.getParentCode())) {//是否为一级分类
				
				ClassifyDto dto = new ClassifyDto();
				BeanUtils.copyProperties(item, dto);//item拷贝到dto中
